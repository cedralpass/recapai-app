...
def organize_taxonomy_result(job_id):
    ...
    for cat in categories:
        cat_type = "merging" if cat.category in merging_from else "unchanged"
        merge_id = next((s["id"] for s in suggestions if cat.category in s["from_categories"]), None)
        current_annotated.append(
            {
                ...
                "type": cat_type,
                "merge_id": merge_id,
                "is_new": cat.category.startswith("New Category:"),
            }
        )
    ...
    merged_results = [
        {
            ...
            "type": "merged",
            "merge_id": s["id"],
            "from_categories": s["from_categories"],
            "from_counts": s["from_counts"],
        }
...
def apply_taxonomy():
    ...
    for old_category, new_category in mapping_to_apply.items():
        articles = Article.query.filter_by(user_id=current_user.id, category=old_category).all()
...
def suggest_splits_result(job_id):
    ...
    session["split_assignments"] = {
        f"split_{i}": suggestions[cat_name]["assignments"] for i, cat_name in enumerate(suggestions.keys())
    }
...
def apply_splits():
    ...
    if has_decision_fields:
        accepted_sids = [
            k[len("accepted_") :] for k, v in request.form.items() if k.startswith("accepted_") and v == "1"
        ]
