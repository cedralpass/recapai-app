...
from recap import Config, db, login_manager
...
class User(UserMixin, db.Model):
    ...
    def get_categories(self):
        groupings = (
            ...
            .filter(Article.user_id == self.id)
    ...
    def verify_reset_password_token(token):
        try:
            id = jwt.decode(token, Config.RECAP_SECRET_KEY, algorithms=["HS256"])["reset_password"]
        except Exception:
            return
...
class Article(db.Model):
    ...
    def get_article_by_url_path(url_path, user_id):
        ...
        stmt = (
            ...
            .where(Article.url_path == url_path)
