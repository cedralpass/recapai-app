...
def parse_args():
    ...
    p.add_argument("--all", action="store_true", help="Reclassify all articles, not just unclassified ones")
    p.add_argument("--delay", type=float, default=2.0, help="Seconds between enqueue calls (default: 2.0)")
    p.add_argument("--dry-run", action="store_true", help="Print articles without enqueuing")
    return p.parse_args()
...
def main():
    ...
    with app.app_context():
        ...
        if not args.all:
            query = query.where(Article.classified is None)
