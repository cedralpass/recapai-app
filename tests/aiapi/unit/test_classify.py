...
class TestClassify:
    def test_unauthorized_access(self, aiapi_client):
        ...
        response = aiapi_client.post("/classify_url", data={"url": "http://example.com", "ref_key": "test-ref-key"})
    ...
    def test_classify_endpoint(self, mock_openai, aiapi_client):
        ...
        mock_response = {
            ...
            "sub_categories": ["Flask Framework", "Application Monitoring", "Debugging Techniques"],
    ...
    def test_build_prompt_with_content(self, aiapi_client):
        ...
        assert (
            "only on the provided text" in messages[0]["content"].lower()
            or "only on the provided" in messages[0]["content"].lower()
        )
        assert messages[1]["role"] == "user"
