...
class TestAuthRedirectGuards:
    ...
    def test_authenticated_user_redirected_from_reset_request(self, authenticated_client):
        ...
        response = authenticated_client.get("/auth/reset_password_request", follow_redirects=False)
        assert response.status_code == 302
...
class TestPasswordResetRequest:
    ...
    def test_reset_request_with_known_email_sends_email(self, mock_send_email, recap_client, recap_app, test_user):
    ...
    @patch("recap.auth.send_password_reset_email")
    def test_reset_request_with_unknown_email_does_not_send_email(self, mock_send_email, recap_client):
    ...
    @patch("recap.auth.send_password_reset_email")
    def test_reset_request_redirects_to_login(self, mock_send_email, recap_client, recap_app, test_user):
    ...
    @patch("recap.auth.send_password_reset_email")
    def test_reset_request_with_invalid_email_does_not_send_email(self, mock_send_email, recap_client):
    ...
    @patch("recap.auth.send_password_reset_email")
    def test_reset_request_mail_failure_is_graceful(self, mock_send_email, recap_client, recap_app, test_user):
...
class TestPasswordReset:
    def test_reset_with_invalid_token_redirects_to_index(self, recap_client):
        ...
        response = recap_client.get("/auth/reset_password/notavalidtoken", follow_redirects=False)
        assert response.status_code == 302
    ...
    def test_authenticated_user_redirected_from_reset_password_token_page(
        ...
        response = authenticated_client.get(f"/auth/reset_password/{token}", follow_redirects=False)
        assert response.status_code == 302
