...
class TestEditProfileRoute:
    def test_get_prepopulates_form_with_current_values(self, authenticated_client, recap_app, test_user):
    ...
    def test_post_duplicate_username_shows_error(self, authenticated_client, recap_app, test_user):
...
class TestOrganizeTaxonomyRoute:
    ...
    def test_organize_taxonomy_returns_200(self, mock_ai, authenticated_client, recap_app, test_user):
    ...
    def test_organize_taxonomy_shows_processing_page(self, authenticated_client, recap_app, test_user):
    ...
    def test_organize_taxonomy_result_shows_ai_description(self, authenticated_client, recap_app, test_user):
    ...
    def test_organize_taxonomy_result_stores_mapping_in_session(self, authenticated_client, recap_app, test_user):
    ...
    def test_organize_taxonomy_result_expired_redirects(self, authenticated_client, recap_app):
        """Result route redirects when the Redis key has expired."""
        with patch.object(recap_app.redis, "get", return_value=None):
            response = authenticated_client.get("/organize_taxonomy/result/fake-job-id", follow_redirects=False)
        assert response.status_code == 302
...
class TestApplyTaxonomyRoute:
    def test_apply_taxonomy_updates_article_categories(self, authenticated_client, recap_app, test_user):
        """POST /apply_taxonomy remaps article categories per the session mapping."""
        with recap_app.app_context():
            article = Article(
                url_path="https://a.com/remap",
                user_id=test_user.id,
                category="Tech",
            )
    ...
    def test_apply_taxonomy_clears_session_mapping(self, authenticated_client, recap_app, test_user):
        """After applying, category_mapping is removed from the session."""
        with recap_app.app_context():
            db.session.add(
                Article(
                    url_path="https://a.com/clear",
                    user_id=test_user.id,
                    category="Tech",
                )
            )
    ...
    def test_apply_taxonomy_without_session_redirects(self, authenticated_client):
...
class TestOrganizeTaxonomyWithSeedData:
    ...
    def test_session_mapping_covers_fixture_categories(self, seeded_authenticated_client, seeded_user, recap_app):
        """
        ...
        assert missing == set(), f"organize_taxonomy session mapping is missing fixture categories: {missing}"
...
class TestApplyTaxonomyWithSeedData:
    ...
    def test_apply_returns_200_and_success_flash(self, seeded_authenticated_client, seeded_user, recap_app):
    ...
    def test_total_article_count_is_unchanged(self, seeded_authenticated_client, seeded_user, recap_app):
        ...
        with recap_app.app_context():
            count = db.session.scalar(sa.select(sa.func.count(Article.id)).where(Article.user_id == seeded_user.id))
        assert count == 52, f"Expected 52 articles after apply, got {count}"

    def test_no_old_categories_remain_after_apply(self, seeded_authenticated_client, seeded_user, recap_app):
        """
        ...
        assert stale == set(), (
            f"These old categories were NOT remapped — apply_taxonomy failed to " f"update them: {stale}"
        )

    def test_merged_categories_combine_article_counts(self, seeded_authenticated_client, seeded_user, recap_app):
        """
        ...
        assert mapping.get("Culinary Arts") == merged_name, (
            f"Expected 'Culinary Arts' to merge into '{merged_name}', " f"got '{mapping.get('Culinary Arts')}'"
        )
    ...
    def test_identity_mapped_categories_preserve_count(self, seeded_authenticated_client, seeded_user, recap_app):
        """
        ...
        for cat in identity:
            ...
            assert got == expected, f"'{cat}' should still have {expected} articles after apply, got {got}"

    def test_session_mapping_is_cleared_after_apply(self, seeded_authenticated_client, seeded_user, recap_app):
    ...
    def test_renamed_category_article_count_matches_original(self, seeded_authenticated_client, seeded_user, recap_app):
        """
        ...
        for new_cat, old_cats in new_category_sources.items():
            if len(old_cats) == 1:  # pure rename, no merge
                ...
                if old_cat != new_cat:
                    ...
                    assert got == expected, f"'{old_cat}' → '{new_cat}': expected {expected} articles, got {got}"
...
class TestSuggestSplitsRoute:
    ...
    def test_suggest_splits_returns_200(self, seeded_authenticated_client, seeded_user, recap_app):
    ...
    def test_suggest_splits_result_shows_new_category_names(self, seeded_authenticated_client, seeded_user, recap_app):
    ...
    def test_suggest_splits_result_expired_redirects(self, seeded_authenticated_client, recap_app):
        """Result route redirects when the Redis key has expired."""
        with patch.object(recap_app.redis, "get", return_value=None):
            response = seeded_authenticated_client.get("/suggest_splits/result/fake-job-id", follow_redirects=False)
        assert response.status_code == 302

    def test_suggest_splits_result_empty_suggestions_redirects(self, seeded_authenticated_client, recap_app):
...
class TestApplySplitsRoute:
    ...
    def test_apply_splits_updates_article_categories(self, seeded_authenticated_client, seeded_user, recap_app):
        ...
        half = len(ai_ids) // 2
        assignments = {str(aid): "LLM Techniques" if i < half else "AI Applications" for i, aid in enumerate(ai_ids)}
        ...
        with recap_app.app_context():
            for i, aid in enumerate(ai_ids):
                ...
                assert (
                    article.category == expected_cat
                ), f"Article {aid} should be '{expected_cat}', got '{article.category}'"

    def test_apply_splits_preserves_total_article_count(self, seeded_authenticated_client, seeded_user, recap_app):
    ...
    def test_apply_splits_clears_session(self, seeded_authenticated_client, seeded_user, recap_app):
...
class TestTaxonomyHelpers:
    ...
    def test_get_categories_with_subcats_returns_sorted_by_count(self, seeded_user, recap_app):
    ...
    def test_get_categories_with_subcats_aggregates_subcats(self, seeded_user, recap_app):
    ...
    def test_build_rich_organize_context_includes_counts_and_subcats(self, seeded_user, recap_app):
