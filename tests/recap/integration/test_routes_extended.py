...
class TestIndexRoute:
    ...
    def test_category_filter_limits_results(self, authenticated_client, recap_app, test_user):
        ...
        with recap_app.app_context():
            db.session.add(
                Article(
                    url_path="https://example.com/tech",
            ...
            db.session.add(
                Article(
                    url_path="https://example.com/science",
                    user_id=test_user.id,
                    category="Science",
                )
            )
...
class TestAddArticleRoute:
    ...
    def test_post_creates_article_and_redirects(self, authenticated_client, recap_app, test_user, mocker):
    ...
    def test_post_persists_article_to_db(self, authenticated_client, recap_app, test_user, mocker):
...
class TestReclassifyRoute:
    def test_reclassify_enqueues_job_and_redirects(self, authenticated_client, recap_app, test_user, mocker):
        """GET /<id>/reclassify enqueues classify_url job and redirects to index."""
        with recap_app.app_context():
            article = Article(url_path="https://example.com/reclassify-me", user_id=test_user.id)
            db.session.add(article)
            db.session.commit()
            article_id = article.id
