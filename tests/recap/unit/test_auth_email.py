...
class TestSendPasswordResetEmail:
    ...
    def test_send_password_reset_email_calls_send_email_with_expected_fields(self, mock_send_email, recap_app):
    ...
    @patch("recap.auth.email.send_email")
    def test_send_password_reset_email_uses_generated_token(self, mock_send_email, mock_render_template, recap_app):
        ...
        mock_render_template.assert_any_call("email/reset_password.txt", user=user, token="generated-token")
        mock_render_template.assert_any_call("email/reset_password.html", user=user, token="generated-token")
        assert mock_send_email.call_count == 1
